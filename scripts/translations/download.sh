...
do
    ...
    node converter.js import messages.json $locale converted.json "tag." || exit 1
    ...
    if [ "$locale" = "zh_CN" ]; then
        echo "Copying tags.json for pt locale"
        cp -f converted.json $workDir/locales/zh/tags.json
    fi
...
do
    ...
    node converter.js import messages.json $locale converted.json "group." || exit 1
    ...
    if [ "$locale" = "zh_CN" ]; then
        echo "Copying groups.json for pt locale"
        cp -f converted.json $workDir/locales/zh/groups.json
    fi
...
do
    ...
    node converter.js import messages.json $locale converted.json "filter." || exit 1
    ...
    if [ "$locale" = "zh_CN" ]; then
        echo "Copying filters.json for pt locale"
        cp -f converted.json $workDir/locales/zh/filters.json
    fi
