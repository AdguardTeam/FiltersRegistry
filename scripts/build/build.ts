...
const filtersDir = path.join(repositoryDir, 'filters');
const logPath = path.join(repositoryDir, 'log.txt');
const platformsPath = path.join(repositoryDir, FOLDER_WITH_NEW_FILTERS);
const copyPlatformsPath = path.join(repositoryDir, FOLDER_WITH_OLD_FILTERS);
const tempDir = path.join(repositoryDir, 'temp');
const cachedRootDir = path.join(tempDir, 'filters_cached');
const cachedFiltersDir = path.join(cachedRootDir, 'filters');
 ...
 *
 * Copies `filters/`, `groups/`, `tags/`, and `locales/` into a self-contained
 * cache root, then replaces the content of every `template.txt` with a single-line
 * local include pointing to the cached `filter.txt`.
 * Validates that every filter directory with a `template.txt` also has a `filter.txt`.
 *
 * @returns {Promise<void>}
 */
const prepareCachedFiltersDir = async (): Promise<void> => {
    // Remove stale copy if exists
    await fs.rm(cachedRootDir, { recursive: true, force: true });
    await fs.mkdir(cachedRootDir, { recursive: true });

    await Promise.all(['filters', 'groups', 'tags', 'locales'].map(async (directory) => {
        await fs.cp(
            path.join(repositoryDir, directory),
            path.join(cachedRootDir, directory),
            { recursive: true },
        );
    }));

    // Find all directories containing template.txt and replace with shadow templates
    const templatePaths = await findFiles(cachedFiltersDir, (p: string) => path.basename(p) === 'template.txt');

    await Promise.all(templatePaths.map(async (templatePath: string) => {
 ...
 */
const buildFilters = async (): Promise<void> => {
    ...
    try {
        ...
        if (useCache && error instanceof OptimizationStatsError) {
            throw Object.assign(
                new Error(`Run --download-stats to download the latest statistics. (${error.message})`),
                { cause: error },
            );
        }
        throw error;
...
buildFilters().catch((error: unknown) => {
