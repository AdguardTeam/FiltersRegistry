...
selection (or was a full, unscoped download); otherwise stats are fetched from the
remote server. Plain `yarn build` always fetches remotely — the local cache is
only applied under `--use-cache`.
