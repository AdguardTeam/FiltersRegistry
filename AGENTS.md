...
| `yarn generate-cache:stats` | Download per-filter `stats.json` files to `temp/optimization_config/`, without recompiling `filter.txt`; use `--include`/`--skip` to scope filters |
