...
- `metadata.json`
    ...
    * <a id="trustLevel"></a> `trustLevel` — level of trust which describe [allowed and permitted rules types](https://github.com/AdguardTeam/FiltersCompiler/tree/master/src/main/utils/trust-levels); possible values:
        ...
        * `ignoreTrustLevel` - note that this directive will allow ignoring the trustLevel for a specific internal file of a third-party filter. The behavior is the same as if `trustLevel` were used with a value of `full`
